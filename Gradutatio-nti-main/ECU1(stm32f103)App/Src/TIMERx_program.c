...
void MTIMERx_voidInit(void)
{
	...
	TIM3->CR1 |= (TIMER_3_DIRECTION << 4); //0
...
void MTIMERx_voidUpdatePSC(uint8_t copy_u8TimerID, uint32_t copy_u32Psc, uint32_t copy_u32MaxValue)
{
...
void MTIMERx_voidSetInputCaptureTrigger(uint8_t copy_u8TimerID, uint8_t copy_u8ChannelID, Trigger_Type copy_u8ICTrigger)
{
	volatile GPTM_t *TIMER = MTIMERx_GetTimerID(copy_u8TimerID);
	switch (copy_u8ChannelID)
	{
...
volatile GPTM_t* MTIMERx_GetTimerID(uint8_t copy_u8TimerID)
{
